...
requireRole(['patient', 'secretaire', 'admin']);

$idMedecinRaw = $_GET['idMedecin'] ?? '';
if ($idMedecinRaw === '' || !ctype_digit((string) $idMedecinRaw)) {
    respond(400, [
        'success' => false,
...
try {
    ...
    $stmt->execute(['idMedecin' => (int) $idMedecinRaw]);
    ...
    error_log('Get public medecin settings error: ' . $e->getMessage());
    respond(500, [
        'success' => false,
